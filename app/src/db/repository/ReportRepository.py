...
class ReportRepository:
    ...
    def _get_study_count(self) -> int:
        ...
        sql = self.cache.load("report/get_study_count.sql")
        return conn.execute(sql).fetchone()

    def _get_patient_count(self) -> int:
        conn = get_connection()
        conn.row_factory = lambda _, row: int(row[0])
        sql = self.cache.load("report/get_patient_count.sql")
        return conn.execute(sql).fetchone()

    def _get_patient_count_by_study(self, study_id: int) -> int:
        conn = get_connection()
        conn.row_factory = lambda _, row: int(row[0])
        sql = self.cache.load("report/get_patient_count_by_study.sql")
        return conn.execute(sql, (study_id,)).fetchone()

    def _get_researcher_count(self) -> int:
        conn = get_connection()
        conn.row_factory = lambda _, row: int(row[0])
        sql = self.cache.load("report/get_researcher_count.sql")
        return conn.execute(sql).fetchone()

    def _get_researcher_count_by_study(self, study_id: int) -> int:
        conn = get_connection()
        conn.row_factory = lambda _, row: int(row[0])
        sql = self.cache.load("report/get_researcher_count_by_study.sql")
        return conn.execute(sql, (study_id,)).fetchone()

    def _get_visit_count(self) -> int:
        conn = get_connection()
        conn.row_factory = lambda _, row: int(row[0])
        sql = self.cache.load("report/get_visit_count.sql")
        return conn.execute(sql).fetchone()

    def _get_visit_count_by_study(self, study_id: int) -> int:
        conn = get_connection()
        conn.row_factory = lambda _, row: int(row[0])
        sql = self.cache.load("report/get_visit_count_by_study.sql")
        return conn.execute(sql, (study_id,)).fetchone()

    def _get_event_count(self) -> int:
        conn = get_connection()
        conn.row_factory = lambda _, row: int(row[0])
        sql = self.cache.load("report/get_event_count.sql")
        return conn.execute(sql).fetchone()

    def _get_event_count_by_study(self, study_id: int) -> int:
        conn = get_connection()
        conn.row_factory = lambda _, row: int(row[0])
        sql = self.cache.load("report/get_event_count_by_study.sql")
        return conn.execute(sql, (study_id,)).fetchone()
    ...
    def _get_studies(self) -> list[dict]:
        ...
        sql = self.cache.load("report/get_studies.sql")
