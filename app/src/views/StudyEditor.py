...
class StudyEditor(View):
    ...
    def study_pane(self):
        with ui.row().classes("mt-2 w-full"):
            ui.button("New", on_click=lambda: self.load(StudyDTO())).props(
                "icon=add"
            ).classes("text-xs")

            ui.button("Save", on_click=lambda: self.save()).props("icon=save").classes(
                "text-xs"
            ).disable().bind_enabled(self.vm, "data_changed")

            ui.button("Undo", on_click=lambda: self.load(self.study)).props(
                "icon=undo"
            ).classes("text-xs")

            ui.space()

            ui.button("Delete", on_click=lambda: ui.notify("Study deleted")).classes(
                "text-xs mr-2"
            ).props("icon=delete").disable()
    ...
    def details_pane(self):
        with (
            ...
            .props("horizontal")
            .classes("p-0")
            .bind_visibility(self.vm, "is_old") as tabs
        ):
            visits = ui.tab("Visits", icon="event").classes("text-sky-800")
            monitoring = ui.tab("Monitoring", icon="monitor_heart").classes(
                "text-sky-800"
            )
            adverse_events = ui.tab("Events", icon="dangerous").classes("text-sky-800")
            patients = ui.tab("Patients", icon="personal_injury").classes(
                "text-sky-800"
            )
